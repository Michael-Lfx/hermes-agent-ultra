...
pub struct AgentConfig {
    ...
    /// De-identified insights contribution to ops server (opt-in).
    #[serde(default)]
    pub insights: hermes_config::InsightsConfig,
...
impl Default for AgentConfig {
    fn default() -> Self {
        Self {
            ...
            insights: hermes_config::InsightsConfig::default(),
...
pub struct AgentLoop {
    ...
    /// Per-session POI buffer (flush at session end; no per-turn SQLite writes by default).
...
impl AgentLoop {
    ...
    fn insights_on_session_end(&self) {
        let cfg = self.config().insights.contribution.clone();
        if !cfg.enabled || !cfg.on_session_end {
            return;
        }
        let hermes_home = self
            .config()
        ...
        if let Some(ref store) = self.interest_store {
            if let Ok(guard) = store.lock() {
                if let Ok(topics) = guard.top_topics(20) {
                    for t in &topics {
                        topic_inputs.push(hermes_insights::InterestTopicInput {
                            id: t.id.clone(),
                            label: t.label.clone(),
        ...
        hermes_insights::ContributionService::spawn_session_end(
            hermes_home,
            cfg,
            topic_inputs,
        );
    ...
    fn spawn_background_review(&self, turn: u32, ctx: &ContextManager, review_memory_at_end: bool) {
        ...
        let review_skills_flag = review_skills;
        let insights_home = self
            .config()
            .hermes_home
            .as_ref()
            .map(std::path::PathBuf::from)
        ...
        tokio::spawn(async move {
            ...
            match agent.run(hist, None).await {
                Ok(result) => {
                    if review_skills_flag {
                        if let Ok(svc) = hermes_insights::ContributionService::open(
                            insights_home.clone(),
                            ...
                            svc.mark_skills_review_patched();
                        }
                    }

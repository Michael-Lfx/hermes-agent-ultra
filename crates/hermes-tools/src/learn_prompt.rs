...
//! [`build_learn_prompt`] and feeds the result to the agent as a normal turn.

/// The house-style rules, distilled from AGENTS.md "Skill authoring standards
/// (HARDLINE)" and the hermes-agent-dev new-skill salvage reference. Embedded
/// in the prompt so the agent authors skills the way a maintainer would by
/// hand.
pub const AUTHORING_STANDARDS: &str = r###"Follow the Hermes skill-authoring standards exactly. These are the same
...
///
/// `user_request` is the free-text the user gave after ``/learn`` — a
...
mod tests {
    ...
    #[test]
    fn test_embeds_the_user_request_verbatim() {
        let req = "the REST client in ~/projects/acme-sdk, focus on auth";
        let prompt = build_learn_prompt(req);
        assert!(prompt.contains(req));
    }

    #[test]
    fn test_always_includes_the_authoring_standards() {
        for req in &["", "a url https://x/y", "what we just did"] {
            assert!(build_learn_prompt(req).contains(AUTHORING_STANDARDS));
        }
    }

    #[test]
    fn test_instructs_saving_via_skill_manage_not_a_raw_file() {
        let prompt = build_learn_prompt("learn the thing");
        assert!(prompt.contains("skill_manage"));
    }

    #[test]
    fn test_references_gather_tools_for_open_ended_sourcing() {
        let prompt = build_learn_prompt("learn from somewhere");
        for tool in &["read_file", "search_files", "web_extract"] {
            assert!(prompt.contains(tool), "prompt should reference {}", tool);
        }
    }

    #[test]
    fn test_empty_request_falls_back_to_the_conversation() {
        let prompt = build_learn_prompt("");
        assert!(prompt.to_lowercase().contains("conversation"));
        assert!(prompt.contains("skill_manage"));
    }
    ...
    #[test]
    fn test_description_length_rule_is_in_the_standards() {
        assert!(AUTHORING_STANDARDS.contains("60"));
    }

    #[test]
    fn test_teaches_the_full_hardline_standards() {
        let std_lower = AUTHORING_STANDARDS.to_lowercase();
        // description: the count-and-trim self-check
        assert!(std_lower.contains("count") && std_lower.contains("60"));
        // platforms gating against OS-bound primitives
        assert!(std_lower.contains("platforms"));
        // author is always the literal Hermes, never the host/OS identity
        assert!(std_lower.contains("author: always the literal value `hermes`"));

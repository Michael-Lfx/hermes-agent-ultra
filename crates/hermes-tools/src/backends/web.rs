    ...
    async fn extract(&self, url: &str, _include_links: bool) -> Result<String, ToolError> {
        ...
        if !status.is_success() {
            let hint = if status.as_u16() == 403 || status.as_u16() == 401 {
                " This site blocks automated access. Prefer answering from existing web_search snippets, \
                 or try another web_search query (e.g. site: or a news-domain query) instead of fetching this URL."
...
// ─────────────────────────────────────────────────────────────────────────────
// DdgsSearchBackend — DDGS + optional CN meta-search (搜狗/必应中国)
// Ported from Python: plugins/web/ddgs/provider.py
// ─────────────────────────────────────────────────────────────────────────────

/// DuckDuckGo search backend powered by the `ddgs` Rust crate.
/// CJK queries also fan out to configured CN HTML engines in parallel.
...
impl WebSearchBackend for DdgsSearchBackend {
    async fn search(
        ...
        crate::backends::meta_search::orchestrator::meta_search(query, num_results).await
    }
}

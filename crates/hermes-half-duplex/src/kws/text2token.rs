        ...
        for word in text.split_whitespace() {
            if let Some(phones) = self.lexicon.get(word) {
                ...
                warn!(
                    word,
                    text,
                    "word not in lexicon and not CJK; skipping phrase"
                );
                return Err(HalfDuplexError::Config(format!(
...
fn load_token_table(path: &str) -> Result<HashSet<String>> {
    let content = std::fs::read_to_string(path)
        .map_err(|e| HalfDuplexError::Config(format!("read tokens {}: {e}", path)))?;
...
fn load_lexicon(path: &str) -> Result<HashMap<String, Vec<String>>> {
    ...
    let content = std::fs::read_to_string(path)
        .map_err(|e| HalfDuplexError::Config(format!("read lexicon {}: {e}", path)))?;

...
pub use shared::{
    OPENAI_CODEX_BASE_URL, codex_cloudflare_headers, openai_codex_provider,
    openai_codex_provider_with_timeout,
};

// ---------------------------------------------------------------------------
// Re-export crate-internal items so other crate modules (provider_serialize_cache,
// tests, etc.) can access them via crate::provider::*
// ---------------------------------------------------------------------------

// Items below are used by submodules (via `super::`) and by `mod tests` (via `super::*`).
// Rust's unused-import lint does not track usage through those paths, so we suppress it.
#[allow(unused_imports)]
pub(crate) use shared::{
    ACP_MULTIMODAL_PREFIX, CODEX_CLOUDFLARE_ORIGINATOR, ChatRequestParams,
    OPENROUTER_RESPONSE_CACHE, OpenRouterResponseCache, OpenRouterResponseCacheControl,
    OpenRouterResponseCacheEntry, anthropic_blocks_from_multimodal_parts,
    build_provider_http_client, clean_moonshot_enum, codex_chatgpt_account_id, flat_model_name,
    flatten_multimodal_parts_text, format_tools_for_openai_api_with_model,
    infer_moonshot_schema_type, is_codex_cloudflare_base_url, is_moonshot_model,
    opencode_go_deepseek_reasoning_effort, opencode_go_kimi_reasoning_effort,
    parse_acp_multimodal_parts, parse_openai_response, parse_sse_chunk, rate_limit_headers_json,
    request_timeout_duration, sanitize_moonshot_schema_node, sanitize_moonshot_tool_parameters,
    sanitize_moonshot_tools_value, summarize_openai_response_shape,
};

// ---------------------------------------------------------------------------
// Tests – all 49 test functions from the original monolithic provider.rs
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    use hermes_core::{FunctionCall, Message, MessageRole, ToolCall, ToolSchema};

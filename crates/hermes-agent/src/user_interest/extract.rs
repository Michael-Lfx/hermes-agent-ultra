...
lazy_static::lazy_static! {
    ...
    .unwrap();
    static ref LANG_RE: Regex = Regex::new(
        r"(?i)\b(rust|python|typescript|javascript|go|java|kotlin|swift|c\+\+|cpp|sql)\b"
    )
    .unwrap();
    static ref TECH_RE: Regex = Regex::new(
        r"(?i)\b(hermes|parity|gateway|mcp|sqlite|tokio|docker|kubernetes|anthropic|claude|openai)\b"
    )
    .unwrap();
}
...
pub fn extract_signals_from_text(text: &str, weight_scale: f64) -> Vec<InterestSignal> {
    let mut out = Vec::new();
    ...
    for cap in LANG_RE.captures_iter(trimmed) {
        if let Some(lang) = cap.get(1) {
            let l = lang.as_str().to_ascii_lowercase();
            if is_poi_blocklisted(&l) {
                continue;
            }
            push_signal(
                &mut out,
                ...
                &l,
                &format!("language: {l}"),
                &format!("Discussed {l} development"),
                0.2 * weight_scale,
                &["lang", &l],
            );
        }
    ...
    for cap in TECH_RE.captures_iter(trimmed) {
        if let Some(term) = cap.get(1) {
            let t = term.as_str().to_ascii_lowercase();
            if !is_acceptable_tech_topic(&t) {
                continue;
            }
            push_signal(
                &mut out,
                ...
                &t,
                &format!("topic: {t}"),
                &format!("Recurring interest in {t}"),
                0.18 * weight_scale,
                &["tech", &t],
            );
        }
...
mod tests {
    ...
    fn chinese_declared_interests_do_not_collide() {
        let a = extract_signals_from_text("我的兴趣点是打篮球", 1.0);
        let b = extract_signals_from_text("我的兴趣点还有吃鱼", 1.0);

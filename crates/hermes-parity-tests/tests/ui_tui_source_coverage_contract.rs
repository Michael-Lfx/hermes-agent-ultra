...
#[test]
fn ui_tui_manifest_covers_source_cluster_and_references_real_tests() {
    let manifest = read_json("docs/parity/ui-tui-source-coverage.json");
    ...
    assert_eq!(
        ...
        "TUI source coverage manifest must cover exactly the ui-tui backlog rows"
    );
    ...
    for entry in manifest_entries {
        ...
        assert_eq!(
            entry["status"].as_str(),
            Some("covered_by_rust_primary_tui_contracts"),
            "{path} should be marked covered_by_rust_primary_tui_contracts"
        );

...
pub fn build_agent_config(config: &GatewayConfig, model: &str) -> AgentConfig {
    ...
    AgentConfig {
        ...
        auxiliary_config: parse_auxiliary_config(&config.auxiliary),
        ..AgentConfig::default()
    }
}

/// Parse the raw `auxiliary` block from `config.yaml` into `AuxiliaryConfig`.
///
/// The yaml supports two formats:
///   - Flat:  `auxiliary: { compression: { fallback_chain: [...] } }`
///   - Structured: `auxiliary: { tasks: { compression: { ... } }, consult_env: true }`
///
/// If the top-level object has no `tasks` key, treat all keys (except
/// `consult_env`) as task names and wrap them under `tasks`.
fn parse_auxiliary_config(
    raw: &Value,
) -> hermes_intelligence::auxiliary::AuxiliaryConfig {
    use hermes_intelligence::auxiliary::AuxiliaryConfig;
    if raw.is_null() {
        return AuxiliaryConfig::default();
    }
    // Try structured format first.
    if let Ok(cfg) = serde_json::from_value::<AuxiliaryConfig>(raw.clone()) {
        return cfg;
    }
    // Flat format: wrap keys into `tasks`.
    if let Some(obj) = raw.as_object() {
        let mut tasks = serde_json::Map::new();
        let mut consult_env = true;
        for (k, v) in obj {
            if k == "consult_env" {
                consult_env = v.as_bool().unwrap_or(true);
            } else {
                tasks.insert(k.clone(), v.clone());
            }
        }
        let wrapped = serde_json::json!({ "tasks": tasks, "consult_env": consult_env });
        if let Ok(cfg) = serde_json::from_value::<AuxiliaryConfig>(wrapped) {
            return cfg;
        }
    }

...
mod platform_impl {
    use super::*;
    use tokio::net::windows::named_pipe::{ServerOptions, NamedPipeServer, ClientOptions};
    ...
    impl IpcListener for WindowsPipeListener {
        async fn accept(&self) -> Result<Box<dyn IpcStream>, IpcError> {
            ...
            let server = self.server_options.create(&self.pipe_path)?;
            server.connect().await?;
            Ok(Box::new(WindowsPipeStream { inner: server }))
        }
    ...
    pub fn create_listener(pipe_path: &str, max_connections: usize) -> Result<Box<dyn IpcListener>, IpcError> {
    ...
    pub async fn poke_listener(pipe_path: &str) {
        ...
        match ClientOptions::new().open(pipe_path) {
            ...
            Err(e) => tracing::debug!(pipe = %pipe_path, error = %e, "poke_listener connect failed (pipe may not exist yet)"),
        }
    }
}
...
mod platform_impl {
    ...
    pub fn create_listener(pipe_path: &str, _max_connections: usize) -> Result<Box<dyn IpcListener>, IpcError> {
    ...
    pub async fn poke_listener(pipe_path: &str) {
        match UnixStream::connect(pipe_path).await {
            Ok(_) => tracing::debug!(pipe = %pipe_path, "poke_listener connected"),
            Err(e) => tracing::debug!(pipe = %pipe_path, error = %e, "poke_listener connect failed (socket may not exist yet)"),
        }
    }
}
...
pub fn default_pipe_path() -> &'static str {
    if cfg!(windows) {
        r"\\.\pipe\AIPC-acp"
    } else {
        "/tmp/hermes-acp.sock"

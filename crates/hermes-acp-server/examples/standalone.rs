...
async fn main() {
    ...
    let executor = match LlmExecutor::from_hermes_config() {
        Some(e) => {
        ...
        None => {
            ...
            eprintln!("Make sure llm_providers.custom is configured with api_key, base_url, model.");
            std::process::exit(1);
        }
    };

    let pipe_path = hermes_acp_server::default_pipe_path().to_string();
